...
> [!NOTE]
> **This fork exists for Claude Code users who want GSD research to work with local models.**
> 
> Upstream GSD research uses paid/cloud APIs and can fall back to Claude Web Search in Claude Code. That works for the standard Claude setup, but breaks down when Claude Code is pointed at a local model through a custom API endpoint such as llama.cpp.
> 
> This fork replaces upstream's default research integrations with a self-hosted stack: `mcp__searxng__*` for web search and `mcp__context__*` for library documentation. Use this fork when you want Claude Code + local research + MCP alignment checks; use upstream when you want the standard published GSD package.
> 
> **What's different here:**
> 
> - **Local-first research.** Research-heavy workflows use SearXNG for web search and a local Context MCP server for library docs, instead of depending on cloud web/docs providers.
> - **Claude Code focused.** This fork is tested primarily with Claude Code, including Claude's cloud models and a local model setup.
> - **Research preflight (`/gsd-research-status`).** Checks whether the local research MCP servers are connected before you run research-heavy workflows.
> - **Tracks upstream.** Upstream features, fixes, and commands are merged periodically; fork-specific changes stay focused on local-first research support.
> 
> **MCP Requirements:** This fork requires two MCP servers to be configured in your `~/.claude/settings.json` or `~/.claude.json`:
> 
> | Server    | Purpose                      | Package                                                    |
> | --------- | ---------------------------- | ---------------------------------------------------------- |
> | `context` | Library documentation lookup | [`@neuledge/context`](https://github.com/neuledge/context) |
> | `searxng` | Web search (no API key)      | [`mcp-searxng`](https://github.com/searxng/searxng)        |
> 
> 
> 
> **Installation:** This fork is only available from source — there is no published npm package for it.
> 
> ```bash
...
> [!NOTE]
> The documentation below is inherited from upstream and covers commands and concepts shared by both projects. Installation instructions in the upstream documentation apply to the published package; use the source install above for this fork.
...
[![Tests](https://img.shields.io/github/actions/workflow/status/gsd-build/get-shit-done/test.yml?branch=main&style=for-the-badge&logo=github&label=Tests)](https://github.com/pedropachecog/get-shit-done/actions/workflows/test.yml)
...
[![GitHub stars](https://img.shields.io/github/stars/gsd-build/get-shit-done?style=for-the-badge&logo=github&color=181717)](https://github.com/pedropachecog/get-shit-done)
...
<details>
<summary><strong>Development Installation (source fork)</strong></summary>

> [!IMPORTANT]
> **`npm run build:hooks` and `npm run build:sdk` are required before installing this fork from source.** The installer copies hook files from `hooks/dist/` and verifies the SDK CLI from `sdk/dist/`. The published upstream npm package handles this automatically via `prepublishOnly`; source installs do not.
...
Add `--minimal` or `--core-only` to install only the core workflow skills. Re-run the build and install commands after changes to `hooks/src/`, `sdk/src/`, or `get-shit-done/`.
